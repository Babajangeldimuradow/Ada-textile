...
@section('title','ADA || Banner Üýtgetmek')
@section('main-content')

<div class="card">
    <h5 class="card-header">Banner üýtgetmek</h5>
    <div class="card-body">
      <form method="post" action="{{route('banner.update',$banner->id)}}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            @if($banner->photo)
                <img src="{{asset('storage/' . $banner->photo)}}" alt="banner photo" style="max-width: 150px; margin-top: 10px;">
            @endif
            @error('photo')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="form-group mb-3">
           <button class="btn btn-success" type="submit">Täzeläň</button>
        </div>
      </form>
    </div>
</div>
