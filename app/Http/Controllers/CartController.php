...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        request()->session()->flash('success','Haryt üstünlikli sebede goşuldy');
        return back();       
    }  
    ...
    public function singleAddToCart(Request $request){
        ...
        if($product->stock <$request->quant[1]){
            return back()->with('error','Haryt gutardy, beýleki önümleri goşup bilersiňiz.');
        }
        if ( ($request->quant[1] < 1) || empty($product) ) {
            request()->session()->flash('error','Nädogry önümler');
            return back();
        }    
    ...
    public function cartUpdate(Request $request){
        ...
        if($request->quant){
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    if($cart->product->stock < $quant){
                        request()->session()->flash('error','Haryt gutardy');
                        return back();
                    }
                    ...
                    // return $cart->price;
                    $cart->save();
                    $success = 'Sebet üstünlikli täzelendi!';
                }else{
                    $error[] = 'Sebet nädogry!';
                }
            }
            ...
            return back()->with('Sebet nädogry!');
        }    
    }
